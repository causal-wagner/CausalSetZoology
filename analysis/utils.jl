...
Draw a node-link plot where nodes are colored by subgraph membership.
Nodes are arranged in separated subgraph clusters to make the partition visible.
If `apply_theme=true`, `apply_paper_theme!` is applied before plotting.
"""
function _draw_subgraph_colored_node_link!(
    ...
    pos = Vector{Makie.Point2f}(undef, n)
    ...
    for i in 1:(n - 1)
        ...
        for j in (i + 1):n
            if A[i, j]
                ...
                lines!(ax, [pi[1], pj[1]], [pi[2], pj[2]], color = (:black, α), linewidth = 1)
            end
        end
    end

    scatter!(ax, first.(pos), last.(pos), color = node_colors, markersize = node_size)
    return ax
end
...
Create a 1×3 row of node-link plots (double-column-sized by default).
"""
function plot_subgraph_colored_node_link(
    A1::BitMatrix,
    A2::BitMatrix,
    ...
    Makie.Box(fig[1, 2], color = :black, strokewidth = 0)
    Makie.Box(fig[1, 4], color = :black, strokewidth = 0)
    Makie.colsize!(fig.layout, 2, Makie.Fixed(8))
    Makie.colsize!(fig.layout, 4, Makie.Fixed(8))
...
"""
    mahalanobis_gap_distinguishability(A, B; regulator=0.0, R=1000, q=0.0, alpha=0.05, rng=..., symmetric=false)
...
function mahalanobis_gap_distinguishability(
    ...
    for r in 1:R
        ...
        sig1 = _mahal_sigmas(B1, mu2, inv2)
        S_base[r] = _summary_stat(sig1, q)
    end
    ...
    if symmetric
        ...
        for r in 1:R
            A1, A2 = _random_split_equal(A, rng)
            mu2s, inv2s = _fit_reference(
                A2,
                regulator;
                stabilization_method = stabilization_method,
                projection_tolerance = projection_tolerance,
            ...
            sig1s = _mahal_sigmas(A1, mu2s, inv2s)
            S_base_sym[r] = _summary_stat(sig1s, q)
        end
...
"""
    scalar_bin_mahalanobis_gap_distinguishability(data::Vector{Vector{Tuple{T,Real}}}; num_bins=nothing, regulator=0.0, R=1000, q=0.0, alpha=0.05, rng=..., symmetric=false)
...
function scalar_bin_mahalanobis_gap_distinguishability(
    ...
    for i in 1:length(bins)
        ...
        for j in (i + 1):length(bins)
            ...
            push!(out, (s1 = s1, s2 = s2, rel_change = relative_change(s1, s2), M_obs = res.M_obs, distinguishable = res.distinguishable, threshold = res.threshold, z_emp = res.z_emp, M_obs_sym = res.M_obs_sym, M_obs_min = res.M_obs_min, threshold_sym = res.threshold_sym, threshold_max = res.threshold_max))
            done += 1
            if progress
                if use_pm
...
"""
    scalar_bin_mahalanobis_gap_distinguishability(data::Vector{Vector{Tuple{T,Real}}}, ref::AbstractVector; num_bins=nothing, regulator=0.0, R=1000, q=0.0, alpha=0.05, rng=..., symmetric=false)
...
function scalar_bin_mahalanobis_gap_distinguishability(
    ...
    for (s, vals) in bins
        ...
        push!(out, (scalar = s, M_obs = res.M_obs, distinguishable = res.distinguishable, threshold = res.threshold, z_emp = res.z_emp, M_obs_sym = res.M_obs_sym, M_obs_min = res.M_obs_min, threshold_sym = res.threshold_sym, threshold_max = res.threshold_max))
        done += 1
        if progress
            if use_pm
                ProgressMeter.next!(pm)
