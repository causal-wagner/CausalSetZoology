...
Like `hist_hist_vec_hist_plot_matrix`, but with a distinguishability fourth panel.
...
function hist_hist_vec_distinguishability_plot_matrix(
    ...
    xlim[4] !== nothing && xlims!(ax4, xlim[4]...)
    ylim[4] !== nothing && ylims!(ax4, ylim[4]...)
    if xt[4] !== nothing
    ...
    lines!(ax4, conn_x, conn_y, label = "connectivity_link")
    lines!(ax4, max_x, max_y, label = "max_pathlen")
    lines!(ax4, ev_x, ev_y, label = "ev_sym_link")
    axislegend(ax4)
