@testsnippet setupGenerateStatistics begin
    ...
    function _fan4_sparse_links()
        return CausalSetZoology.SparseLinksCauset(
            Int64(4),
            [Int32[2, 3, 4], Int32[], Int32[], Int32[]],
            [Int32[], Int32[1], Int32[1], Int32[1]],
        )
    end
...
@testitem "generate_statistics: compute_statistics basic" setup=[setupGenerateStatistics] begin
    ...
    @test hasfield(typeof(rec), :height_distribution_hist)
    ...
    @test sum(values(rec.height_distribution_hist)) == rec.n
...
@testitem "generate_statistics: compute_statistics observables one-by-one" setup=[setupGenerateStatistics] begin
    ...
    expected = Dict(
        ...
        :height_distribution => (:height_distribution_hist, [:degree_hist, :degree_hist_link, :cardinalities_hist, :max_pathlen_hist]),
...
@testitem "generate_statistics: compute_statistics links-only" setup=[setupGenerateStatistics] begin
    ...
    rec = CausalSetZoology.compute_statistics(
        links;
        kind = "grid",
        observables = [:link_degree, :max_pathlen, :max_pathlen_sources, :height_distribution, :ev_sym, :ev_antisym, :communicability],
    )
    ...
    @test rec.height_distribution_hist == Dict(0 => 1, 1 => 1, 2 => 1, 3 => 1)
end

# Verifies height_distribution is the past-boundary counterpart of max_pathlen.
@testitem "generate_statistics: height_distribution asymmetric fan" setup=[setupGenerateStatistics] begin
    links = _fan4_sparse_links()
    rec = CausalSetZoology.compute_statistics(
        links;
        kind = "grid",
        observables = [:max_pathlen, :height_distribution],
    )

    @test rec.max_pathlen_hist == Dict(1 => 1, 0 => 3)
    @test rec.height_distribution_hist == Dict(0 => 1, 1 => 3)
end
...
@testitem "generate_statistics: compute_statistics links-only equivalence" setup=[setupGenerateStatistics] begin
    cset, links = _generated_fixture(kind = "grid")
    obs = [:link_degree, :max_pathlen, :max_pathlen_sources, :height_distribution, :ev_sym, :ev_antisym, :communicability]
